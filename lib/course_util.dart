...
List<CourseModel> getSortedVaildCourses(List<CourseModel> courses) {
  ...
  /// `start` should not be -1, `step` value -1 is used to construct [EmptyBox]
  list.removeWhere((course) {
    return (course.start < 0) ||
        (0 == course.weeks.length) ||
...
List getFilledCourses({
  ...
  // fill with empty block if there is space in the end
